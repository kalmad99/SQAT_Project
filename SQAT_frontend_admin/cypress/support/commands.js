...
Cypress.Commands.add(
  "loginExistingAdmin",
  (existingAdminEmail, existingAdminPassword) => {
...
Cypress.Commands.add(
  ...
  (incorrectemail, incorrectpassword) => {
    return cy.document().then({ timeout: 560000 }, (doc) => {
      ...
      cy.get('[data-cy="login-button"]', { timeout: 70000 }).click();
      cy.wait("@validateAdminLogin", { timeout: 70000 })
        .its("response.statusCode")
        .should("eq", 404); // And here your code.
      // cy.url({ timeout: 70000 }).should('eq', 'http://localhost:3000/')
    });
  }
);
...
Cypress.Commands.add("logout", () => {
  return cy.document().then({ timeout: 560000 }, (doc) => {
    ...
    cy.url({ timeout: 1000 }).should("eq", "http://localhost:3000/admin/login");
  });
});
